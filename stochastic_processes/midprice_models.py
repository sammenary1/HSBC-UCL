...
class AlphaAdverseSelectionMidpriceModel(MidpriceModel):
    def __init__(
        self,
    ...
    def update(self, arrivals: np.ndarray, fills: np.ndarray, actions: np.ndarray, state: np.ndarray = None) -> np.ndarray:
        dW = sqrt(self.step_size) * self.rng.normal(size=(self.num_trajectories, 1))
        dW_alpha = sqrt(self.step_size) * self.rng.normal(size=(self.num_trajectories, 1))
        ...
        # Calculating ε^+ and ε^- 
        epsilon_scale_factor = 100 # INFLUENCES THE EFFECT OF THE ALPHAS (HIGHER = MORE EFFECT)
        epsilon_plus = epsilon_scale_factor * np.abs(self.rng.normal(size=(self.num_trajectories, 1))) # Positve random variable
        epsilon_minus = -epsilon_scale_factor * np.abs(self.rng.normal(size=(self.num_trajectories, 1))) # Negative random variable
        ...
        # Midprice update
        dM = (
            (self.long_term_drift + self.alpha) * self.step_size * np.ones((self.num_trajectories, 1))
            + self.volatility * dW
        )

        #print(f'before self.current_state \n{self.current_state}')
