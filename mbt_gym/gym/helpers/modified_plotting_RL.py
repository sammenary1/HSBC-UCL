...
from mbt_gym.gym.helpers.generate_trajectory_RL import generate_trajectory_RL


def plot_trajectory_RL(env: gym.Env, agent: Agent, seed: int = None):
    timestamps = get_timestamps(env)
    env.original_obs, actions, rewards = generate_trajectory_RL(env, agent, seed)
    ...
    ax1.plot(timestamps[1:], cum_rewards[0, :])
    ax2.plot(timestamps, asset_prices[0, :])
    ax3.plot(
        timestamps,
        inventory[0, :],
        label="inventory",
...
def generate_results_table_and_hist(vec_env: TradingEnvironment, agent: Agent, n_episodes: int = 1000):
    ...
    env.original_obs, actions, rewards = generate_trajectory_RL(vec_env, agent)
