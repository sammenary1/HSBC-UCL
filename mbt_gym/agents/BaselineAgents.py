...
class CarteaJaimungalMmAgentAdverseSelection(Agent):
    def __init__(
            self,
            env: TradingEnvironment = None,
            max_inventory: int = 100,
        ...
        assert isinstance(self.env.reward_function, (CjMmCriterion, PnL)), "Reward function for CjMmAgent is incorrect."
        self.kappa = self.env.model_dynamics.fill_probability_model.fill_exponent
        self.jump_size = self.env.model_dynamics.midprice_model.jump_size
        self.num_trajectories = self.env.num_trajectories
        if isinstance(self.env.reward_function, PnL):
            self.inventory_neutral = True
            self.risk_neutral_action = 1 / self.kappa + self.jump_size * np.ones((env.num_trajectories, env.action_space.shape[0]))
        else:
            self.inventory_neutral = False
            self.phi = env.reward_function.per_step_inventory_aversion
            self.alpha = env.reward_function.terminal_inventory_aversion
    ...
    def _calculate_deltas(self, current_time: float, inventories: np.ndarray):
        ...
        max_inventory_ask = h_minus_one == h_0
        deltas[:, BID_INDEX] = (
                1 / self.kappa + self.jump_size - h_plus_one + h_0 + self.large_depth * max_inventory_bid).reshape(
            -1)
        deltas[:, ASK_INDEX] = (
                1 / self.kappa + self.jump_size - h_minus_one + h_0 + self.large_depth * max_inventory_ask).reshape(
            -1)
        return deltas
    ...
    def _calculate_a_and_z(self):
        ...
        for i in range(matrix_size):
            inventory = self.max_inventory - i
            # Amatrix[i, i] = -self.phi * self.kappa * inventory ** 2
            Amatrix[i, i] = inventory * self.kappa * (self.jump_size * self.lambdas[BID_INDEX]
                                                      - self.jump_size * self.lambdas[ASK_INDEX]) - self.phi * self.kappa * inventory ** 2

            z_vector[i, 0] = np.exp(-self.alpha * self.kappa * inventory ** 2)
            if i + 1 < matrix_size:
                Amatrix[i, i + 1] = self.lambdas[BID_INDEX] * np.exp(-1 - self.kappa*self.jump_size)
            if i > 0:
                Amatrix[i, i - 1] = self.lambdas[ASK_INDEX] * np.exp(-1 - self.kappa*self.jump_size)
        return Amatrix, z_vector
...
class StochasticCarteaJaimungalMmAgent(Agent):
    def __init__(
        self,
        env: TradingEnvironment = None,
        max_inventory: int = 100,
        ...
        # Make sure the fill probability model is StochasticExponentialFillFunction
        assert isinstance(self.env.model_dynamics.fill_probability_model, StochasticExponentialFillFunction), "Fill probability model must be StochasticExponentialFillFunction"

        # Access the kappa attribute from the fill probability model
        self.kappa = self.env.model_dynamics.fill_probability_model.kappa

        self.num_trajectories = self.env.num_trajectories
        if isinstance(self.env.reward_function, PnL):
            self.inventory_neutral = True
            # Calculate the risk-neutral action for inventory-neutral case
            self.risk_neutral_action = 1.0 / self.kappa * np.ones((env.num_trajectories, env.action_space.shape[0]))
        else:
            self.inventory_neutral = False
            self.phi = env.reward_function.per_step_inventory_aversion
            self.alpha = env.reward_function.terminal_inventory_aversion
    ...
    def get_action(self, state: np.ndarray):
        if isinstance(self.env.model_dynamics.fill_probability_model, StochasticExponentialFillFunction):
            # Update kappa (current_state) for StochasticExponentialFillFunction
            self.update_kappa(state)
    ...
    def update_kappa(self, state: np.ndarray):
        # Assuming the fill_probability_model has the stochastic process update logic similar to ExponentialFillFunction
        # You can adjust the arguments passed to the update method based on the actual requirements
        arrivals = np.zeros((self.num_trajectories, 2))
        fills = np.zeros((self.num_trajectories, 2))
        actions = np.zeros((self.num_trajectories, 2))
        self.env.model_dynamics.fill_probability_model.update(arrivals, fills, actions, state)
...
class StochasticCarteaJaimungalMmAgent(Agent): #works with stochastic kappa value
    def __init__(
        self,
        env: TradingEnvironment = None,
        max_inventory: int = 100,
    ...
    def get_action(self, state: np.ndarray):
        ...
        if self.inventory_neutral:
            raise NotImplementedError("Inventory-neutral case is not supported for StochasticExponentialFillFunction.")
        else:
            assert (
                state[0, TIME_INDEX] == state[-1, TIME_INDEX]
            ), "CarteaJaimungalMmAgent needs to be called on a tensor with a uniform time stamp."
    ...
    def _calculate_deltas(self, current_time: float, inventories: np.ndarray):
        ...
        # Calculate deltas for both sides using indexing
        # Made everything 2x2 to work with kappa, so need to talk index to find delta
        deltas[:, BID_INDEX] = (1 / self.kappa[:, BID_INDEX] - h_plus_one[:, 0] + h_0[:, 0] + self.large_depth * max_inventory_bid[:, 0]).flatten()
        deltas[:, ASK_INDEX] = (1 / self.kappa[:, ASK_INDEX] - h_minus_one[:, 0] + h_0[:, 0] + self.large_depth * max_inventory_ask[:, 0]).flatten()
    ...
    def _calculate_a_and_z(self):
        matrix_size = 2 * self.max_inventory + 1
        Amatrix = np.zeros(shape=(2 * matrix_size, 2 * matrix_size))  # Double the size for two-sided fill function
        z_vector = np.zeros(shape=(2 * matrix_size, 1))  # Double the size for two-sided fill function
        for i in range(matrix_size):
            inventory = self.max_inventory - i
            Amatrix[i, i] = -self.phi * self.kappa[:, BID_INDEX] * inventory**2
            z_vector[i, 0] = np.exp(-self.alpha * self.kappa[:, BID_INDEX] * inventory**2)
            if i + 1 < matrix_size:
                Amatrix[i, i + 1] = self.lambdas[BID_INDEX] * np.exp(-1)
            if i > 0:
                Amatrix[i, i - 1] = self.lambdas[ASK_INDEX] * np.exp(-1)

            # For the ask side, shift the indices by matrix_size to fill the second half of the matrix
            inventory = i - self.max_inventory
            Amatrix[i + matrix_size, i + matrix_size] = -self.phi * self.kappa[:, ASK_INDEX] * inventory**2
            z_vector[i + matrix_size, 0] = np.exp(-self.alpha * self.kappa[:, ASK_INDEX] * inventory**2)
            if i + 1 < matrix_size:
                Amatrix[i + matrix_size, i + matrix_size + 1] = self.lambdas[ASK_INDEX] * np.exp(-1)
            if i > 0:
                Amatrix[i + matrix_size, i + matrix_size - 1] = self.lambdas[BID_INDEX] * np.exp(-1)
